...
class StartRunnerGUI(tk.Frame):
    ...
    def __init__(self, master, *args, **kwargs):
        ...
        self.parent.iconbitmap(default="images" + os.sep + "RLBot_logo.ico")
    ...
    def start_running(self):
        ...
        agent_headers = [custom_config.ConfigObject.ConfigHeader() for i in range(num_bots)]
        ...
        for t, team in enumerate([self.blue_agents, self.orange_agents]):
            for agent in team:
                ...
                agent_headers[i].add_value("agent_module", str, var=agent.agent_path.get().replace(
                    os.path.dirname(os.path.realpath(__file__)).replace(os.sep, "/"), "", 1).replace("/", ".")[1:-3])
                i += 1
        while i < 10:
        ...
        self.forget()
        self.parent.destroy()
        rocketleaguerunner.main(framework_config=rlbotcfg, bot_agent_headers=agent_headers)
